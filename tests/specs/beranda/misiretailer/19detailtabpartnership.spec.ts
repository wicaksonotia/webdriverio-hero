...
When(/^I search for weekend then click tab Partnership$/, async () => {
    // await MisiRetailerScreen.search({toko: 'weekend'});
});

Then(/^system should be show the details of Partnership tab$/, async () => {
    await MisiRetailerScreen.Partnership();
});